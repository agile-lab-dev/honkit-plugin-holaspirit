...
import { HolaspiritClient } from "./holaspirit";

dotenv.config();

function isValidRoleBlock(roleBlock: string) {
...
function parseRoleBlock(roleBlock: string) {
  let splittedRoleBlock = roleBlock.split("/");

  let parentCircleName =
    splittedRoleBlock[splittedRoleBlock.length - 2].toLowerCase();
  ...
  hooks: {
    init: async function () {
      const organizationId = this.config.get(
        "pluginsConfig.holaspirit.organizationId"
      );
      if (!organizationId) {
  ...
  blocks: {
    role: {
      process: async function (block) {
        ...
        const link = await holaspirit.getRoleLink(
          circlesMap,
          parsedRoleBlock.circleName,
          parsedRoleBlock.roleName
        );
